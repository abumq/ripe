...
echo
valgrind --leak-check=full ./ripe -g --rsa --out-private private.pem --out-public public.pem
echo
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> RSA Encryption"
echo
echo 'plain text' | valgrind --leak-check=full ./ripe -e --rsa --in-key public.pem --out output.enc
echo
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> RSA Decryption"
echo
valgrind --leak-check=full ./ripe -d --rsa --in-key private.pem --in output.enc --base64
echo
