...
echo
valgrind ./ripe -g --rsa --out-private private.pem --out-public public.pem --leak-check=full --show-leak-kinds=all
echo
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> RSA Encryption"
echo
echo 'plain text' | valgrind ./ripe -e --rsa --in-key public.pem --out output.enc --leak-check=full --show-leak-kinds=all
echo
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> RSA Decryption"
echo
valgrind ./ripe -d --rsa --in-key private.pem --in output.enc --base64 --leak-check=full --show-leak-kinds=all
echo
