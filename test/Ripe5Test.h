...
// p, q, e, msg, expected
static TestData<Ripe5::BigInteger, Ripe5::BigInteger, unsigned int, std::string, std::string> RSADetailedEncryption = {
    //TestCase(11, 17, 3, "Hi", std::vector<long long int> { 183, 95 }),
    //TestCase(53, 59, 3, "Hi", std::vector<long long int> { 1135, 635 }),
    //TestCase(60779, 53003, 65537, "Hi", std::vector<long long int> { 569814661, 1696284635 }),
    //TestCase("", "", 3, "Hi", std::vector<long long int> { 373248, 1157625 }),
    TestCase(Ripe5::BigInteger("108215449534587396558557488943879350166773359647071667116025080873441234413887"), Ripe5::BigInteger("91243493758612676931094271904842149664289633274572930135618242638207523081573"), 3, "Hi", "374883342557625")
};
...
TEST(Ripe5Test, DetailedEncryption)
{
    for (const auto& item : RSADetailedEncryption) {
        LOG(INFO) << "Generating key ... bit(p) = "
                  << PARAM(0).BitCount() << ", bit(q) = "
                  << PARAM(1).BitCount();
        Ripe5::KeyPair k(PARAM(0), PARAM(1), PARAM(2));
        LOG(INFO) << "Encrypting ...";
        //ASSERT_EQ(Ripe5::encrypt(k.publicKey(), PARAM(3), Ripe5::PaddingType::NO_PADDING), PARAM(4));
        LOG(INFO) << "Decrypting ...";
        //ASSERT_EQ(Ripe5::decrypt(k.privateKey(), PARAM(4), Ripe5::PaddingType::NO_PADDING), PARAM(3));
        LOG(INFO) << "Done!";

        std::cout << Ripe5::encrypt(k.publicKey(), "Hi") << std::endl;
    }
}
